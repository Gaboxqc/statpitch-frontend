...
function TeamCrest({ name, url, className = 'w-6 h-6' }: TeamCrestProps) {
  if (url) {
    return (
      <img src={url} alt={`${name} crest`} className={`${className} object-contain rounded-sm`} />
    )
  }
  ...
  return (
    <div
      ...
      className={`${className} shrink-0 flex items-center justify-center rounded-sm bg-accent/50 border border-secondary-foreground/15 text-secondary-foreground font-bold`}
    >
